...
export async function GET(
  ...
  try {
    ...
    const subcategories_query = await db.execute(
      sql`select ${subcategories.id} from ${subcategories} where ${subcategories.categoryId} in (select ${categories.id} from ${categories} where ${categories.storeId} = ${key} and slug = ${slug})`
    );
