import { pgTable, pgEnum, pgSchema, AnyPgColumn, foreignKey, serial, varchar, integer, timestamp, text, doublePrecision, boolean, date } from "drizzle-orm/pg-core"
...
export const clothes = pgTable("clothes", {
	...
	storeId: integer("store_id").default(1).references(() => stores.id),
});
...
export const orders = pgTable("orders", {
	...
	shippingAddressId: integer("shipping_address_id").notNull().references(() => address.id),
	billingAddressId: integer("billing_address_id").notNull().references(() => address.id),
	status: varchar("status", { length: 255 }).notNull(),
	shippingProviderName: varchar("shipping_provider_name", { length: 255 }).notNull(),
	shippingProviderType: varchar("shipping_provider_type", { length: 255 }).notNull(),
...
export const variants = pgTable("variants", {
	...
	createdAt: date("created_at").default(CURRENT_DATE).notNull(),
	updatedAt: date("updated_at").default(CURRENT_DATE).notNull(),
	clothesId: integer("clothes_id").notNull().references(() => clothes.id),
});
...
export const stores = pgTable("stores", {
	id: integer("id").default(nextval('stores_id_seq'::regclass)).primaryKey().notNull(),
